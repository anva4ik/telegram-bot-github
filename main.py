...
def start(m):
    bot.send_message(
        ...
        "ищу в интернете, генерирую изображения\n"
        "и со временем становлюсь умнее.\n\n"
        "Пиши 👇",
        parse_mode="Markdown"
    )
...
def web_search(q):
    ...
    d = r.json()
    return "\n\n".join([f"{i['name']}\n{i['url']}" for i in d.get("webPages",{}).get("value",[])])

def search_image(q):
    h={"Authorization":PEXELS_API_KEY}
    r=requests.get("https://api.pexels.com/v1/search",headers=h,params={"query":q,"per_page":1})
    d=r.json()
...
def chat(m):
    ...
    if text.lower().startswith("найди"):
        bot.send_message(m.chat.id, web_search(text))
        return
